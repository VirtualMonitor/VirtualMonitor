...
int VBoxDispMPDisablePointer(PVBOXDISPDEV pDev)
{
    ...
    dwrc = EngDeviceIoControl(pDev->hDriver, IOCTL_VIDEO_DISABLE_POINTER, NULL, 0, NULL, 0, &cbReturned);
    VBOX_CHECK_WINERR_RETRC(dwrc, VERR_DEV_IO_ERROR);
    pDev->pointerX = -1;
    pDev->pointerY = -1;
    if (pDev->pPointerMoveEvent)
        EngSetEvent(pDev->pPointerMoveEvent);
...
int VBoxDispMPSetPointerPosition(PVBOXDISPDEV pDev, PVIDEO_POINTER_POSITION pPos)
{
    ...
    dwrc = EngDeviceIoControl(pDev->hDriver, IOCTL_VIDEO_SET_POINTER_POSITION, pPos, sizeof(VIDEO_POINTER_POSITION),
                              NULL, 0, &cbReturned);
    VBOX_CHECK_WINERR_RETRC(dwrc, VERR_DEV_IO_ERROR);
    pDev->pointerX = pPos->Column;
    pDev->pointerY = pPos->Row;
    if (pDev->pPointerMoveEvent)
        EngSetEvent(pDev->pPointerMoveEvent);
...
int VBoxDispMPSetPointerAttrs(PVBOXDISPDEV pDev)
{
    ...
    if (pDev->pPointerShapeEvent)
        EngSetEvent(pDev->pPointerShapeEvent);
