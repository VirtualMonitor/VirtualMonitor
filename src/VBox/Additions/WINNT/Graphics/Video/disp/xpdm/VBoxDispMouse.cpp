...
int VBoxDispInitPointerAttrs(PVBOXDISPDEV pDev)
{
    ...
    pDev->pointer.pAttrs = (PVIDEO_POINTER_ATTRIBUTES) EngAllocMem(FL_NONPAGED_MEMORY, pDev->pointer.cbAttrs, MEM_ALLOC_TAG);
...
VOID APIENTRY VBoxDispDrvMovePointer(SURFOBJ *pso, LONG x, LONG y, RECTL *prcl)
{
    ...
    if (-1==x) /* hide pointer */
    {
        rc = VBoxDispMPDisablePointer(pDev);
        VBOX_WARNRC(rc);
    }
    else
    {
        ...
        rc = VBoxDispMPSetPointerPosition(pDev, &pos);
        VBOX_WARNRC(rc);
    }
