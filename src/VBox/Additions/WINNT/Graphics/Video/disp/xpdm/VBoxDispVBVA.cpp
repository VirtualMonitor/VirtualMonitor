...
*/
        ...
        if (pDev->pPaintEvent) {
            pDev->dirtyRect[pDev->wIndex].top = rect.top; 
            pDev->dirtyRect[pDev->wIndex].bottom = rect.bottom; 
            pDev->dirtyRect[pDev->wIndex].left = rect.left; 
            pDev->dirtyRect[pDev->wIndex].right = rect.right; 
            pDev->wIndex++;
            if (pDev->wIndex >= MAX_DIRTY_RECT) {
            pDev->dirtyRect[0].top = 0; 
            pDev->dirtyRect[0].bottom = pDev->mode.ulHeight; 
            pDev->dirtyRect[0].left = 0; 
            pDev->dirtyRect[0].right = pDev->mode.ulWidth; 
		...
		LOGF(("OverWrite"));
            ...
            LOGF(("dest t: %d, b: %d l: %d r: %d wi: %d, ri: %d",
                 rect.top, rect.bottom, rect.left, rect.right, pDev->wIndex, pDev->rIndex));
            EngSetEvent(pDev->pPaintEvent);
        }
