...
VOID APIENTRY VBoxDispDrvDisablePDEV(DHPDEV dhpdev)
{
    ...
    if (pDev->pRestartEvent) {
        EngSetEvent(pDev->pRestartEvent);
    }
...
ULONG APIENTRY VBoxDispDrvEscape(SURFOBJ *pso, ULONG iEsc, ULONG cjIn, PVOID pvIn, ULONG cjOut, PVOID pvOut)
{
    ...
    switch (iEsc)
    {
        ...
        case VM_CTL_SET_EVENT:
        {
            if (!pvIn || cjIn < sizeof(VM_EVENT_ARRAY)) {
                WARN(("%s: %d:\n", __FUNCTION__, __LINE__));
                return 0;
            }
            pEventArray = (PVM_EVENT_ARRAY)pvIn;

            pDev->hPaintEvent = pEventArray->hPaintEvent;
            pDev->pPaintEvent = EngMapEvent(pDev->hDevGDI,
                            pDev->hPaintEvent,
                            NULL, NULL, NULL);
                    
            pDev->hRestartEvent = pEventArray->hRestartEvent;
            pDev->pRestartEvent = EngMapEvent(pDev->hDevGDI,
                            pDev->hRestartEvent,
                            NULL, NULL, NULL);
        
            pDev->hPointerShapeEvent = pEventArray->hPointerShapeEvent;
            pDev->pPointerShapeEvent = EngMapEvent(pDev->hDevGDI,
                                pDev->hPointerShapeEvent,
                                NULL, NULL, NULL);

            pDev->hPointerMoveEvent = pEventArray->hPointerMoveEvent;
            pDev->pPointerMoveEvent = EngMapEvent(pDev->hDevGDI,
                                pDev->hPointerMoveEvent,
                                NULL, NULL, NULL);
                return 1;
        }
	    case VM_CTL_UNSET_EVENT:
        {
            if (!pvIn || cjIn < sizeof(VM_EVENT_ARRAY)) {
                WARN(("%s: %d:\n", __FUNCTION__, __LINE__));
                return 0;
            ...
            if ((pDev->hPaintEvent == pEventArray->hPaintEvent) &&
                (pDev->pPaintEvent)) {
                EngUnmapEvent(pDev->pPaintEvent);
                            pDev->pPaintEvent = NULL;
                pDev->hPaintEvent = NULL; 
            }
            if ((pDev->hPointerShapeEvent == pEventArray->hPointerShapeEvent) &&
                (pDev->pPointerShapeEvent)) {
                EngUnmapEvent(pDev->pPointerMoveEvent);
                pDev->pPointerShapeEvent = NULL;
            }
            if ((pDev->hPointerMoveEvent == pEventArray->hPointerMoveEvent) &&
                (pDev->pPointerMoveEvent)) {
                EngUnmapEvent(pDev->pPointerMoveEvent);
                pDev->pPointerMoveEvent = NULL;
            }
            return 1;
        }
        case VM_CTL_GET_DIRTY_EARA:
        {
            if (!pvOut || cjOut < sizeof(RECT)) {
                WARN(("%s: %d:\n", __FUNCTION__, __LINE__));
                return 0;
            }
            EngClearEvent(pDev->pPaintEvent);
            if (pDev->rIndex == pDev->wIndex) {
                return 0;
            }
#if 1
            RECT   *pRect = (PRECT)pvOut;
            pRect->left =pRect->right= pRect->top= pRect->bottom = 0;
            *pRect = pDev->dirtyRect[pDev->rIndex];
            pDev->rIndex++;
            if (pDev->rIndex >= MAX_DIRTY_RECT) {
                pDev->rIndex = 0;
            }
        ...
        case VM_CTL_UPDATE_DIRTY_EARA: 
        {
            INT lineSize = (pDev->mode.ulWidth *  pDev->mode.ulBitsPerPel) >> 3;
            ULONG Size = pDev->mode.ulHeight * lineSize;
            ULONG copied = 0;
            if (!pvIn || cjIn < sizeof(RECT)) {
                WARN(("%s: %d:\n", __FUNCTION__, __LINE__));
                return 0;
            }
            if (!pvOut || cjOut < Size) {
            ...
            RECT   *pRect = (PRECT)pvIn;
#if 0
            WARN(("Up Event: left: %d, right: %d, top: %d, bottom: %d\n", pRect->left, pRect->right, pRect->top, pRect->bottom));
            LOG(("w: %d, h: %d, BPP: %d, lineSize: %d, Size: %d\n", 
                pDev->mode.ulWidth, pDev->mode.ulHeight, pDev->mode.ulBitsPerPel, lineSize, Size));
#endif
#if 1
            INT offset = (pRect->left * pDev->mode.ulBitsPerPel) >> 3; 
            INT copyLineSize = ((pRect->right - pRect->left) * pDev->mode.ulBitsPerPel)  >> 3;

            PCHAR src = (PCHAR)pDev->memInfo.FrameBufferBase + pRect->top*lineSize + offset;
            // PCHAR dst = (PCHAR)pvOut + pRect->top*lineSize + offset;
            PCHAR dst = (PCHAR)pvOut;
            for (INT y = pRect->top; y < pRect->bottom; y++) {
                if ((dst + copyLineSize <= (PCHAR)pvOut + Size) && 
                    (src+copyLineSize<=(PCHAR)pDev->memInfo.FrameBufferBase+Size)) { 
                            memcpy(dst, src, copyLineSize); 
                } else {
                    LOG(("Bad, offset: %d, y: %d %d, %x, %x, %x, %x\n", offset, y, y-pRect->top, 
                    dst + copyLineSize, (PCHAR)pvOut + Size, (src+copyLineSize), (PCHAR)pDev->memInfo.FrameBufferBase+Size));
                }
                copied += copyLineSize;
                dst += copyLineSize;
                src += lineSize;
        ...
        case VM_CTL_GET_POINTER_SHAPE_EVENT:
        {
            COLORPOINTER *p;
            uint32_t fFlags;
            uint32_t cbData = 0;
            ...
            if (!pvOut || cjOut < pDev->pointer.cbAttrs) {
                return -1;
            }
            p = (COLORPOINTER*)pvOut;
            fFlags = pAttrs->Enable & 0x0000FFFF;

            if (fFlags & VBOX_MOUSE_POINTER_SHAPE)
            {
                /* Size of the pointer data: sizeof (AND mask) + sizeof (XOR_MASK) */
                cbData = ((((pAttrs->Width + 7) / 8) * pAttrs->Height + 3) & ~3)
                         + pAttrs->Width * 4 * pAttrs->Height;
                /* If shape is supplied, then always create the pointer visible.
                 * See comments in 'vboxUpdatePointerShape'
                 */
                fFlags |= VBOX_MOUSE_POINTER_VISIBLE;
            }
            p->fAlpha = ((fFlags & VBOX_MOUSE_POINTER_ALPHA) != 0);
            p->fVisible = ((fFlags & VBOX_MOUSE_POINTER_VISIBLE) != 0);
            p->u16HotX = (pAttrs->Enable >> 16) & 0xFF;
            p->u16HotY = (pAttrs->Enable >> 24) & 0xFF;
            p->u16Width = (uint16_t)pAttrs->Width;
            p->u16Height = (uint16_t)pAttrs->Height;
            memcpy(p->data, pAttrs->Pixels, pDev->pointer.cbAttrs - sizeof(VIDEO_POINTER_ATTRIBUTES));
            return sizeof(*p);
        }
        case VM_CTL_GET_POINTER_MOVE_EVENT: 
        {
            PointerMove *p;
