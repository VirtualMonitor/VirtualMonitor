                             ...
                             PVIDEO_MEMORY_INFORMATION pMapInfo, PSTATUS_BLOCK pStatus)
{
    ...
    ULONG size = VBoxMPXpdmCurrentVideoMode(pExt)->VisScreenHeight * VBoxMPXpdmCurrentVideoMode(pExt)->ScreenStride *2;
    pMapInfo->VideoRamBase = VideoPortAllocatePool(pExt, VpNonPagedPool, size, 'zyxD');
    if (pMapInfo->VideoRamBase)
    {
	pStatus->Status = NO_ERROR;
        pMapInfo->FrameBufferBase = (PUCHAR)pMapInfo->VideoRamBase;
        pMapInfo->FrameBufferLength = size;
        pExt->pPrimary->u.primary.ulMaxFrameBufferSize = size;
#if 0
...
#endif
    VideoPortFreePool(pExt, VideoMemory->RequestedVirtualAddress);
    pStatus->Status = NO_ERROR;
    LOGF_LEAVE();
    return TRUE;
}
