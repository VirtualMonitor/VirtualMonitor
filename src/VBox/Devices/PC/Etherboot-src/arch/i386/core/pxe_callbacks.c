...
#ifdef VBOX
	__asm__ ( ".globl " STRINGIFY(GSYM(_pxe_stack_t_size)) );
	__asm__ ( ".equ " STRINGIFY(GSYM(_pxe_stack_t_size)) ", %c0"
...
#ifdef VBOX
__asm__ ( STRINGIFY(GSYM(pxe_stack_size)) ": .long " STRINGIFY(GSYM(_pxe_stack_size)) );
#else /* !VBOX */
...
int xstartpxe ( void ) {
	 ...
	 */
        ...
        RM_FRAGMENT(jump_to_pxe_nbp,
                /* Clear registers, just to be on the safe side. Clients
                 * may get confused if they switch to protected mode and
                 * use the full 32 bits accidentally. */
                "xorl %eax,%eax\n\t"
                "xorl %ebx,%ebx\n\t"
                "xorl %ecx,%ecx\n\t"
                "xorl %edx,%edx\n\t"
                ...
                "popw %bx\n\t"
                "popw %es\n\t"
                "lcall $" STRINGIFY(PXE_LOAD_SEGMENT) ", $" STRINGIFY(PXE_LOAD_OFFSET) "\n\t"
        );
#else /* !VBOX */
	RM_FRAGMENT(jump_to_pxe_nbp,
		"popw %bx\n\t"
...
#ifdef VBOX
__asm__ ( ".globl " STRINGIFY(GSYM(_pxe_stack_t_size)) );
__asm__ ( ".equ " STRINGIFY(GSYM(_pxe_stack_t_size)) ", 0" );
#else /* !VBOX */
__asm__ ( ".globl _pxe_stack_t_size" );
