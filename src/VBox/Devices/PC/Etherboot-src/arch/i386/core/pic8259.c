...
RM_FRAGMENT(_trivial_irq_handler,
	...
	"incw  %cs:(" STRINGIFY(GSYM(_trivial_irq_trigger_count)) "-1b)(%bx)\n\t" 
	"popw  %bx\n\t" 
	"iret\n\t" 
	"\n\t"
	".globl " STRINGIFY(GSYM(_trivial_irq_trigger_count)) "\n\t"
	STRINGIFY(GSYM(_trivial_irq_trigger_count)) ": .short 0\n\t"
	"\n\t"
	".globl " STRINGIFY(GSYM(_trivial_irq_chain_to)) "\n\t"
	STRINGIFY(GSYM(_trivial_irq_chain_to)) ": .short 0,0\n\t"
	"\n\t"
	".globl " STRINGIFY(GSYM(_trivial_irq_chain)) "\n\t"
	STRINGIFY(GSYM(_trivial_irq_chain)) ": .byte 0\n\t"
	);
#else /* !VBOX */
RM_FRAGMENT(_trivial_irq_handler,
	"pushw %bx\n\t"
