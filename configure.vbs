...
function CheckForlibSDLSub(strPathlibSDL)
   ...
   if   LogFileExists(strPathlibSDL, "lib/" & archDir & "/SDL.lib") _
    And LogFileExists(strPathlibSDL, "lib/" & archDir & "/SDLmain.lib") _
    And LogFileExists(strPathlibSDL, "lib/" & archDir & "/SDL.dll") _
...
''
' Checks for any libSDL binaries.
sub CheckForlibJPEG(strOptlibJPEG)
   dim strPathlibJPEG, str
   PrintHdr "libJPEG"

   '
   ' Try find some JPEG library.
   '

   ' First, the specific location.
   strPathlibJPEG = ""
   if (strPathlibJPEG = "") And (strOptlibJPEG <> "") then
      if CheckForlibJPEGSub(strOptlibJPEG) then strPathlibJPEG = strOptlibJPEG
   end if

   ' Success?
   if strPathlibJPEG = "" then
      if strOptlibJPEG = "" then
         MsgError "Can't locate libJPEG. Try specify the path with the --with-libJPEG=<path> argument. " _
                & "If still no luck, consult the configure.log and the build requirements."
      else
         MsgError "Can't locate libJPEG. Please consult the configure.log and the build requirements."
      end if
      exit sub
   end if

   strPathLibJPEG = UnixSlashes(PathAbs(strPathLibJPEG))
   CfgPrint "PATH_SDK_LIBJPEG       := " & strPathlibJPEG

   PrintResult "libJPEG", strPathlibJPEG
end sub

''
' Checks if the specified path points to an usable libSDL or not.
function CheckForlibJPEGSub(strPathlibJPEG)
   CheckForlibJPEGSub = False
   LogPrint "trying: strPathlibJPEG =" & strPathlibJPEG
   if   LogFileExists(strPathlibJPEG, "lib/turbojpeg-static.lib") then
      CheckForlibJPEGSub = True
   end if
end function

sub ConfigLibVnc()
	strRfbConfigFile = "src/libs/libvnc-0.9.9/rfb/rfbconfig.h"
	FileDelete strRfbConfigFile
   	FileAppendLine strRfbConfigFile, "#ifndef _RFB_RFBCONFIG_H"
	FileAppendLine strRfbConfigFile, "#define _RFB_RFBCONFIG_H 1"
	FileAppendLine strRfbConfigFile, "/* Generated automatically at end of configure. */"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_ALLOW24BPP  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_ALLOW24BPP  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_GETHOSTBYNAME  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_GETHOSTNAME  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_INET_NTOA  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_LIBPNG  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_LIBZ  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_MEMMOVE  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_MEMORY_H  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_MEMSET  1"
        FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_FCNTL_H 1"
	'FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_NETINET_IN_H  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_SELECT  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_SOCKET  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_STDLIB_H  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_STRINGS_H  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_STRING_H  1"
	'FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_SYS_SOCKET_H  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_SYS_TYPES_H  1"
	'FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_UNISTD_H  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_WS2TCPIP_H 1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_IPv6  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_PACKAGE  ""LibVNCServer"""
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_PACKAGE_BUGREPORT  ""http://sourceforge.net/projects/libvncserver"""
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_PACKAGE_STRING  ""LibVNCServer 0.9.9"""
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_PACKAGE_TARNAME  ""libvncserver"""
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_PACKAGE_VERSION  ""0.9.9"""
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_VERSION  ""0.9.9"""
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_HAVE_LIBJPEG 1"
	'FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_WITH_TIGHTVNC_FILETRANSFER  1"
	FileAppendLine strRfbConfigFile, "#define LIBVNCSERVER_WITH_WEBSOCKETS  1"
	FileAppendLine strRfbConfigFile, ""
	FileAppendLine strRfbConfigFile, "#endif"

	strRfbConfigFile = "src/libs/libvnc-0.9.9/rfb/rfbint.h"
	FileDelete strRfbConfigFile
	FileAppendLine strRfbConfigFile, "#ifndef _RFB_RFBINT_H"
	FileAppendLine strRfbConfigFile, "#define _RFB_RFBINT_H 1"
	FileAppendLine strRfbConfigFile, "#ifndef _GENERATED_STDINT_H"
	FileAppendLine strRfbConfigFile, "#define _GENERATED_STDINT_H ""LibVNCServer 0.9.9"""
	FileAppendLine strRfbConfigFile, "/* Generated automatically at end of configure. */"
	FileAppendLine strRfbConfigFile, "#include <iprt/stdint.h>"
	FileAppendLine strRfbConfigFile, "#endif"
	FileAppendLine strRfbConfigFile, "#endif"
end sub
